...
class Request
{
   ...
   private function perfomLogging()
   {
       ...
       if(!is_null(self::$logs) && !is_null(self::$log))
       {
    ...
    public static function executeRequest_old($url,$data)
    {
    ...
    /**
     * Method pour verifier si une donnée obligatoire a été envoyée
     * @param $key
     * @param array $data
     */
    public static function checkRequiredData($module,$key,array $data)
    {
        if(!isset($data[$key]))
        {
            throw new Errors($module,$key." : obligatoire.",200);
        }
    }

    /**
     * Method pour vérifier si une donnée obligatoire a été envoyée via POST
     * @param $module
     * @param $key
     * @throws Errors
     * @throws Exception
     */
    public static function checkPOSTRequiredData($module,$key)
    {
        if(Request::isPost())
        {
            if(!isset($_POST[$key]))
        ...
        {
            throw new Exception("Bad check Request");
        }
    }

    /**
     * Method pour verifier si une donnée obligatoire a été envoyé en GET
     * @param $module
     * @param $key
     * @throws Errors
     * @throws Exception
     */
    public static function checkGETRequiredData($module,$key)
    {
        if(Request::isGet())
        {
            if(!isset($_GET[$key]))
